  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <PageHeader
        icon={BarChart3}
        title="챔피언 통계"
        description={`${patch ? `패치 ${patchLabel(patch)}` : "전체"} · 수집된 솔로랭크 ${stats.totalGames.toLocaleString()}경기 표본 기준${stats.computedAt ? ` · ${computedAgo(stats.computedAt)} 집계` : ""} · 챔피언을 누르면 상세 페이지로 이동해요`}
      />
      <ChampionsTable
        stats={stats}
        version={version}
