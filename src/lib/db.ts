...
const SCHEMA_SQL = `
    ...
    LANGUAGE sql IMMUTABLE STRICT AS $fn$
      SELECT coalesce(string_agg(
        CASE WHEN c BETWEEN 44032 AND 55203 THEN
          ...
          || (ARRAY['ㅏ','ㅐ','ㅑ','ㅒ','ㅓ','ㅔ','ㅕ','ㅖ','ㅗ','ㅗㅏ','ㅗㅐ','ㅗㅣ','ㅛ','ㅜ','ㅜㅓ','ㅜㅔ','ㅜㅣ','ㅠ','ㅡ','ㅡㅣ','ㅣ'])[(((c - 44032) % 588) / 28) + 1]
          || (ARRAY['','ㄱ','ㄲ','ㄱㅅ','ㄴ','ㄴㅈ','ㄴㅎ','ㄷ','ㄹ','ㄹㄱ','ㄹㅁ','ㄹㅂ','ㄹㅅ','ㄹㅌ','ㄹㅍ','ㄹㅎ','ㅁ','ㅂ','ㅂㅅ','ㅅ','ㅆ','ㅇ','ㅈ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ'])[((c - 44032) % 28) + 1]
        ELSE chr(c) END, '' ORDER BY ord), '')
      FROM (SELECT ascii(ch) AS c, ord FROM regexp_split_to_table(t, '') WITH ORDINALITY AS x(ch, ord)) s
    $fn$;
    ...
    CREATE TABLE IF NOT EXISTS match_participants (
      ...
      patch text,
      rank_pts real,
      PRIMARY KEY (fp, match_id, player_id)
    );
    ...
    CREATE INDEX IF NOT EXISTS mp_player_time_idx ON match_participants (fp, player_id, game_creation DESC);
